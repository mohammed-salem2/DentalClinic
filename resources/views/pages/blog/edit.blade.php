...
@endsection
@section('content')
    <form id="form" method="POST" action="{{route('blog.update' , $blog->id)}}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card card-custom">
            ...
            <div class="card-body">
                    ...
                    <div class="col-12 mb-5">
                        <div class="d-flex">
                            <div class="symbol symbol-60 symbol-xxl-100 align-self-start align-self-xxl-center">
                                <div class="col-9">
                                    <div class="image-input image-input-empty image-input-outline" id="kt_user_edit_avatar" style="background-image: url('{{asset($blog->logo)}}');background-color: #CCCCCC">
                ...
                <input name="title" value="{{$blog->title}}" type="text" class="form-control form-control-lg @error('title') is-invalid @enderror mb-5" autocomplete="off"/>
                @error('title') <span class="invalid-feedback">{{$message}}</span> @enderror
                <div id="kt_quil_1" style="height: 325px">
                    {!! $blog->subject !!}
                </div>
                <textarea name="subject" id="subject" hidden>{{$blog->subject}}</textarea>
            </div>
        </div>
    </form>
@endsection

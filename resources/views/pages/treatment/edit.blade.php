...
@section('body')
    <form id="form" method="POST" action="{{route('treatment.update' , $treatment->id)}}">
        @csrf
        @method('PUT')
        <div class="card-body">
            <div class="row form-group user-container">
                <div class="col-4 mb-5">
                    <div class="d-flex flex-stack mb-2">
                        <label class="form-label font-weight-bolder text-dark fs-6 mb-0 user-select-none">Medicine</label>
                    </div>
                    <input name="medicine" maxlength="40" value="{{$treatment->medicine}}" type="text" class="form-control form-control-lg @error('medicine') is-invalid @enderror" autocomplete="off" required/>
                ...
                <div class="col-4 mb-5">
                    ...
                    <input name="date" type="date" maxlength="20" value="{{$treatment->date}}" class="form-control form-control-lg @error('date') is-invalid @enderror" autocomplete="off" required/>
                ...
                <div class="col-4 mb-5">
                    ...
                    <select name="patient_id" class="form-control form-control-lg selectpicker" required>
                        @foreach($patients as $patient)
                            <option value="{{$patient->id}}" {{$patient->id == $treatment->patient_id ? 'selected' : ''}}>{{$patient->name}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-12">
                    ...
                    <textarea class="details @error('details') is-invalid @enderror" maxlength="1000" name="details" cols="194" rows="6" autofocus>{{$treatment->details}}</textarea>
